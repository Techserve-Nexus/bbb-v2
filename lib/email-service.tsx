...
export async function sendViasendGrid(to: string, registrationId: string, name: string, htmlContent: string) {
  const response = await fetch("https://api.sendgrid.com/v3/mail/send", {
    ...
    body: JSON.stringify({
      ...
      from: {
        email: process.env.SENDGRID_FROM_EMAIL || "bbbshreeparashurama@gmail.com",
      ...
      reply_to: {
        email: process.env.SENDGRID_REPLY_TO || "bbbshreeparashurama@gmail.com",
      },
      // Optional: headers to indicate preferred reply-to
      headers: {
        "Reply-To": process.env.SENDGRID_REPLY_TO || "bbbshreeparashurama@gmail.com",
      },
      content: [
        {
          type: "text/html",
